...
APIKEY = 'lOJyD4BjeDQn0aTAXxR5uuHaAxGeKvEt9Q3SK7YsTX4e'
...
def before_space(s):
    ...
    # check preconditions
    assert type(s) == str, "type is not string"
    assert introcs.find_str(s," ") != -1, "no space in the string"
...
def first_inside_quotes(s):
    ...
    # check preconditions
    assert type(s) == str, "type is not string"
    assert introcs.count_str(s,'"') >= 2, "need at least 2 double quotes"
...
def get_src(json):
    ...
    # check preconditions
    assert type(json) == str, "type is not string"

    # get result
    index_src_end = introcs.find_str(json,'"src":') + 5
    result = first_inside_quotes(json[index_src_end + 1:])
    return result
...
def get_dst(json):
    ...
    # get result
    index_dst_end = introcs.find_str(json,'"dst":') + 5
    result = first_inside_quotes(json[index_dst_end + 1:])
    return result
...
def has_error(json):
    ...
    # get result
    index_error_end = introcs.find_str(json,'"error":') + 7
    result = first_inside_quotes(json[index_error_end + 1:]) != ""
    return result
...
def service_response(src,dst,amt):
    ...
    assert (type(amt) == int or type(amt) == float), 'type is not int or float'

    currency1 = src
    currency2 = dst
    amount = str(amt)
    
    q = ('https://ecpyfac.ecornell.com/python/currency/fixed?src=' + currency1 +
    '&dst=' + currency2 + '&amt=' + amount + '&key=' + APIKEY)
...
def iscurrency(currency):
    ...
    json = service_response(currency,'TWD',2.5)
    result = has_error(json) != True
    return result
...
def exchange(src,dst,amt):
    ...
    # Calc
    # 1. Connect to the web service and get the json message
    s = service_response(src,dst,amt)
    # 2. Get result of dst
    dst = get_dst(s)
    # 3. Parse out the dst amount
    dst_amt = float(before_space(dst))
    # 4. Return dst amount
    return dst_amt
